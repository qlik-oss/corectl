...
func Eval(ctx context.Context, doc *enigma.Doc, args []string) {
	...
	if err != nil {
		log.Fatalf("could not get hypercube layout: %s", err)
	}
	...
	if len(layout.HyperCube.DimensionInfo) != 0 && layout.HyperCube.DimensionInfo[0].Error != nil {
		log.Fatalf("could not evaluate expression, error returned code: %d", layout.HyperCube.DimensionInfo[0].Error.ErrorCode)
	}
