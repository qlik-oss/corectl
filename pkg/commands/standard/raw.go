...
func CreateRawCommand() *cobra.Command {
	command := engine.WithLocalFlags(&cobra.Command{
		...
		Short:             "Send Http API Request to Qlik Sense SaaS",
		Long:              "Send Http API Request to Qlik Sense Saas. Query parameters are specified using the --query flag, a body can be specified using one of the body flags (body, body-file or body-values)",
